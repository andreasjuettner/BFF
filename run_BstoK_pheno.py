...
##############################################################################
import lib.physical_constants_BstoK as pc
...
import lib.BFF_pheno_lib as Bff_pheno
...
input_dict = {
	 ...
	 #'tstar':	str((pc.mBsphys+pc.mKphys)**2), #'29.349570696829012', # value of t*
	 'tstar':	str((pc.mBphys+pc.mpiphys)**2), #'29.349570696829012', # value of t*
	 ...
	 #'mpole0':	[5.63],		# fzero pole (no pole for BstoK)
...
new = 0
if new == 1:
	 input_dict['tstar' ]=	str((pc.mBsphys+pc.mKphys)**2) #'29.349570696829012', # value of t*
	 input_dict['mpole0']:	[5.63]		# fzero pole (no pole for BstoK)
else:
	 input_dict['tstar' ]:	str((pc.mBphys+pc.mpiphys)**2)  #'29.349570696829012', # value of t*
	 input_dict['mpole0']:	[]		# fzero pole (no pole for BstoK)
...
# bff 		= Bff.Bff(input_dict)
 bff 		= Bff_pheno.Bff_pheno(input_dict,large_t=True)
 ...
 os.system('cp Report/BFF_pheno_report.pdf Reports/BFF_pheno_report_'+sbffl+'.pdf')
